...
function makeMiddleware (setup) {
  return function multerMiddleware (req, res, next) {
    ...
    busboy.on('file', function (fieldname, fileStream, filename, encoding, mimetype) {
      ...
      const parseBlob = parseBlobs && filename && filename.toLowerCase() === 'blob'
      if (!filename || parseBlob) { // default filename value for "Blobs" https://developer.mozilla.org/en-US/docs/Web/API/FormData/append
        ...
        switch (mimetype) {
          ...
          default:
            // if no filename or if we do not parse filenames with the value "blob", output warning
            if (!filename || !parseBlob) {
