  ...
  return function multerMiddleware (req, res, next) {
    ...
    busboy.on('file', function (fieldname, fileStream, filename, encoding, mimetype) {
      ...
      if (!filename || (parseBlobs && filename === 'blob')) { // default filename value for "Blobs" https://developer.mozilla.org/en-US/docs/Web/API/FormData/append
        console.log("NEW NON-FILE", filename, fileStream, encoding, mimetype);
        ...
        const data = new Promise(resolve => {
          ...
          switch (mimetype) {
            case 'text/csv':
              return appendField(req.body, fieldname, data)
            case 'application/json':
              try {
                const parsedField = JSON.parse(data)
                return appendField(req.body, fieldname,)
              } catch (err) {
                console.warn('failed parsing json in ', fieldname, err)
              }
            default:
              console.warn('ignored multipart with fieldname', fieldname)
